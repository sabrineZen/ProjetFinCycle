...
// On importe l'image ici pour que le chemin soit correct au build
import logoImage from "../assets/Design-logo .png";

const Navbar = ({ isOutOfVideo, setMontrerPanier, plats, setTexte, navigate }) => {
  return (
    <div className={`fixed top-0 left-0 right-0 z-[100] flex items-center transition-all duration-500 
      ...
      {/* --- LOGO "P" REMPLACÉ PAR L'IMAGE --- */}
      {!isOutOfVideo && (
        <motion.div 
          initial={{ x: -20, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
          className="flex-shrink-0 text-white font-black text-xl bg-white/20 w-10 h-10 rounded-full flex items-center justify-center backdrop-blur-md border border-white/30 mr-2 overflow-hidden"
        >
          <img 
            src={logoImage} 
            alt="Logo" 
      ...
      >
        ...
        <div className="flex items-center gap-1 md:gap-3">
          <motion.button whileTap={{ scale: 0.9 }} onClick={() => setMontrerPanier(true)} className="relative p-1">
            <FaShoppingCart size={18} />
            <span className="absolute -top-1 -right-1 text-[8px] w-4 h-4 rounded-full flex items-center justify-center font-bold bg-orange-600 text-white border border-white">2</span>
          </motion.button>
          <motion.button whileTap={{ scale: 0.9 }} onClick={() => navigate("/profil")} className="p-1">
            <FaUser size={16} />
          </motion.button>
        </div>
      </motion.div>
