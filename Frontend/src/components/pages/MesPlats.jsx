...
const MesPlats = () => {
  ...
  // Utilisation de platsData dans un state pour permettre les modifications (suppression/visibilité)
  ...
  const categories = ['Tous', 'Burger', 'Pizza', 'Salade', 'Desserts', 'Boissons'];
  const platsFiltrés = filter === 'Tous' ? platsData : platsData.filter(p => p.categorie === filter);

  // --- FONCTIONNALITÉS AJOUTÉES ---

  // Inverser la disponibilité (Masquer/Activer)
  const handleToggleStatus = (id) => {
    setPlatsData(prev => 
      prev.map(plat => plat.id === id ? { ...plat, disponible: !plat.disponible } : plat)
    );
  };
    ...
    <div className="relative min-h-screen">
      ...
      <div className="bg-white p-4 rounded-[20px] shadow-xl flex flex-wrap items-center justify-between gap-6 mb-10">
        <div className="flex items-center gap-3">
          {categories.map((cat) => (
            <button key={cat} onClick={() => setFilter(cat)} className={`px-7 py-2 rounded-xl font-regular text-sm transition-all ${filter === cat ? 'bg-[#FF843D] text-white shadow-lg' : 'bg-[#FFE8D6]/40 text-[#951418] border-1 border-[#C0A0A0]'}`}>
              {cat}
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-8">
            ...
            <div className="p-3 flex-1 flex flex-col">
              ...
              <div className="flex gap-3 mb-6">
                <button 
                  ...
                  className="flex-1 flex items-center justify-center shadow-lg gap-2 py-4 rounded-2xl font-regular text-xs text-white bg-[#FF843D] hover:scale-105 transition-transform"
                >
                ...
                <button 
                  ...
                  className="p-4 bg-[#FF7D31] text-white rounded-2xl shaadow-lg hover:bg-red-600 transition-colors"
                >
                  <Trash2 size={20} />
                </button>
              </div>
        ...
        <button onClick={handleAdd} className="border-2 shadow-xl border-orange-100 rounded-[25px] flex flex-col items-center justify-center gap-6 p-8 bg-[#FFE3CE] hover:border-[#FF843D] transition-all">
          <div className="w-20 h-20 bg-[#FF843D] rounded-3xl flex items-center justify-center text-white shadow-xl"><Plus size={40} strokeWidth={3} /></div>
          <span className="text-[#951418] font-regular text-xl">Ajouter un plat</span>
        </button>
      </div>
