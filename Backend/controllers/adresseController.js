const { Adresse, Utilisateur } = require('../models')
...
function getMesAdresses(req, res) {
  ...
  .then(utilisateur => {
    return Adresse.findAll({ where: { utilisateurId: req.user.id } })
    .then(adresses => {
      const adresseInscription = {
        id: null,
        type: 'domicile',
        rue: utilisateur.adresse,
        ville: '',
        pays: '',
        parDefaut: true,
        nom: utilisateur.prenom + ' ' + utilisateur.nom
      }
      res.status(200).json([adresseInscription, ...adresses])
    })
  })
  .catch(error => {
